...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.funds.update", [$fund->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="asset_name">{{ trans('cruds.fund.fields.asset_name') }}</label>
                <input class="form-control {{ $errors->has('asset_name') ? 'is-invalid' : '' }}" type="text" name="asset_name" id="asset_name" value="{{ old('asset_name', $fund->asset_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('asset_category') ? 'is-invalid' : '' }}" type="text" name="asset_category" id="asset_category" value="{{ old('asset_category', $fund->asset_category) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $fund->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Fund::TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ old('type', $fund->type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Fund::MADE_BY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('made_by') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="made_by_{{ $key }}" name="made_by" value="{{ $key }}" {{ old('made_by', $fund->made_by) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $fund->date) }}" required>
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
